...
public class Level4 extends Level
{
	...
	private final int NBRICK_ROWS = 10;
		...
		for (int rowID = 0; rowID < NBRICK_ROWS; rowID++)
			for (int columnID = 0; columnID < NBRICKS_PER_ROW; columnID++)
			{
					GImage image = new GImage(
							(rowID < NBRICK_ROWS/2.)? info.BRICK_NAMES[3] : info.BRICK_NAMES[2],
							BRICK_SEP + columnID * (BRICK_SEP+brickWidth) + RandomGenerator.getInstance().nextDouble(-8, 8),
							BRICK_Y_OFFSET + rowID * (BRICK_SEP+info.BRICK_HEIGHT) + RandomGenerator.getInstance().nextDouble(-8, 8));
					
					image.setSize(brickWidth,info.BRICK_HEIGHT);
					
					bricks.add(new Brick(image, null,(rowID < NBRICK_ROWS/2.)? 3 : 2));
			}
